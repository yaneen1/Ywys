<?php
$Token = "6916358932:AAGiXV1HwK4SETc1U2f8o56MvcAAAUrJBU8";//توكنك//
$admin = 5519603769;// ايديك //
...
if($text =="م" and in_array($from_id,$adminss)){
bot('sendmessage',[
'chat_id'=>$chat_id,
...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != 5519603769) {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$from_id = $update->callback_query->from->id;
    ...
    bot('answerCallbackQuery',[
      'callback_query_id'=>$update->callback_query->id,
      'text'=>"انتظر 3 ثواني قبل ان تضغط مره آخرى ⏱",
      'show_alert'=>true
      ]);
     exit;
  }
...
$adm = [ 
  'inline_keyboard'=>[
    [['text'=>"فتح الهديه اليومية",'callback_data'=>"onhdia" ], ['text'=>"قفل الهديه اليومية",'callback_data'=>"ofhdia" ]], 
...
$admnb = [ 
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == 5519603769 ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
          'message_id'=>$message_id,
...
SETJSON($rshq);
  ...
  $chabot = $rshq['cha']; if ($chabot == null){$chabot = "bb2mmm";}
...
SETJSON($rshq);
  ...
  $nambot = $rshq['namebot']; if($nambot == null){$nambot = "كريستال";}
...
if($text and $rshq['mode'][$from_id] == "hdiMk") {
   ...
   'chat_id'=>$chat_id,
   'text'=>"
   *تم انشاء كود هدية بوت @Roshko1Bot  🎁*
•الكود💳 : `". $mts."`
 •الكمية 💠: $text1
•عدد الاشخاص 👤: $text
...
- الدخول للبوت : @Roshko1Bot 
- أضغط على استخدام الكود💳
-أرسل الكود ➕
...
$RBEROO = [
  'inline_keyboard'=>[
    [['text'=>"عدد الطلبات الكلي┇$bot_tlb 💎",'callback_data'=>"myrders"]],
    [['text'=>"الخدمات 🛒",'callback_data'=>"service"]],
     [['text'=>"تجميع رصيد ➕",'callback_data'=>"plus" ], ['text'=>"حسابي 📃",'callback_data'=>"acc" ]],
     [['text'=>"استخدام كود 💳",'callback_data'=>"hdia" ], ['text'=>"تحويل نقاط ♻️",'callback_data'=>"transer" ]],
     [['text'=>"طلبات حسابك  〽️",'callback_data'=>"myrders" ]],
     [['text'=>"شحن نقاط 💰",'callback_data'=>"buy" ],['text'=>"فحص طلب 📤",'callback_data'=>"infotlb" ]],

      [['text'=>"قراءة الشروط ⚠️",'callback_data'=>"termss" ]],
      [['text'=>"قناة البوت 💬",'url'=>"$chabot.t.me" ]],
   ]
  ];
$rshq = json_decode(file_get_contents("RSHQ/rshq.json"),true);
...
bot('editmessagetext',[
  ...
  'message_id' => $message_id,
  'text'=>"
  ☆ مرحبا عزيزي : [$name](tg://user?id=$chat_id)  .
  ☆ هذا هي طلباتك 
  *MUHAMMAD*
 ", 
 'parse_mode'=>"markdown",
...
bot('sendMessage',[
   'chat_id'=>$chat_id,
   'text'=>"
*• اهـلآ بك في   $nambot  ☑️*
*• رشق فيسبوك & انستغرام &  تليكرام & تيك توك & وكل ما يخص السوشيال ميديا *
...
'text'=>"
☆ مرحبا عزيزي : [$name](tg://user?id=$chat_id)  .
☆ يمكنك تجميع النقاط بالطرق التالية .
",
'parse_mode'=>"markdown",
'reply_markup'=>json_encode([
     'inline_keyboard'=>[
...
*———————×———————*
رصيدك : $coin نقطه
الرصيد المستخدم : (".($rshq["cointlb"][$from_id] ?? "0").") نقطه
لقد دعوت : $share ♥
...
'reply_markup'=>json_encode([
     'inline_keyboard'=>[
     [['text'=>"تجميع رصيد $b",'callback_data'=>"plus" ], ['text'=>"$HDIAS",'callback_data'=>"hdiaa" ]],
     [['text'=>"$NamesBACK",'callback_data'=>"tobot" ]],
...
if(preg_match('/start Bero/',$text)){
   ...
   'chat_id'=>$chat_id,
   'text'=>"
 لقد حصلت على *". $rshq['thoiler'][$e1]["coin"]. "* رصيد من رابط التحويل
  ", 
  'parse_mode'=>"markdown",
  'reply_markup'=>json_encode([
     'inline_keyboard'=>[
...
'reply_markup'=>json_encode([
     'inline_keyboard'=>[
     [['text'=>"تجميع رصيد",'callback_data'=>"plus" ], ['text'=>"$HDIAS",'callback_data'=>"hdiaa" ]],
     [['text'=>"$NamesBACK",'callback_data'=>"tobot" ]],
...
*• ايديك : *`$from_id` 🆔
*• رصيدك :  $coin 💰*
*🛒︙يرجى إختيار إحد الاقسام من الأسفل..*
",
'parse_mode'=>"markdown",
'reply_markup'=>json_encode($key),
]);
...
'text'=>"
".$rshq['WSFV'][$from_id]."
• ارسل الرابط الخاص بيك
",
'reply_markup'=>json_encode([ 
  'inline_keyboard'=>[
  
  [['text'=>'الغاء' ,'callback_data'=>"tobon"]],
  ]])
]);
...
العدد ". $rshq['3dd'][$from_id][$from_id] . " $tp
 عدد الطلبات : *". $rshqaft." * 
بوت رشق كريستال : @Roshko1Bot
- - - - - - - - - - - - - - - - - - 
  ", 
 'parse_mode'=>"markdown",
 'reply_markup'=>json_encode([
     'inline_keyboard'=>[
     [['text'=>"رشق كريستال 💎",'url'=>"https://t.me/Roshko1Bot"]],
